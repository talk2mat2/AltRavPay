export class pageLinks {
	static readonly UploadRequest = "/upload-request"
	static readonly UploadDocument = "/upload-document"
	static readonly "Audit Logs" = "/audit_logs"
	static readonly Settings = "/settings"
	static readonly Users = "/users"
	static readonly NewUsers = "/users/new"
	static readonly guid = "/guid"
	static readonly Dashboard = "/overView"
	static readonly Overview = "/overView"
	static readonly Reports = "/reports"
	static readonly Request = "/request"
	static readonly Branch = "/branches"
	static readonly NewBranch = "/branch/new"
	static readonly BranchExcelData = "/branch/excel_data"
	
}
